...
import agc.playground.cookingrecipe.data.mapper.toDomainPage
...
class RecipeRepositoryImpl(private val api: RecipeApi) : RecipeRepository {
    ...
    override suspend fun getAllRecipes(limit: Int, skip: Int): Result<Page<Recipe>> =
        runCatching { api.getRecipes(limit, skip).toDomainPage() }

    override suspend fun getRecipe(recipeId: String): Result<Recipe> =
        runCatching { api.getRecipe(recipeId.toInt()).toDomain() }
    ...
    override suspend fun searchRecipesByText(
        ...
        runCatching { api.searchRecipes(searchQuery, limit, skip).toDomainPage() }

    override suspend fun searchRecipesByTag(tag: Tag): Result<List<Recipe>> =
        runCatching { api.getRecipesByTag(tag.label).recipes.map { it.toDomain() } }
}
